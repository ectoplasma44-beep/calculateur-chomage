...
export const metadata: Metadata = {
  title: "Chômage intérimaire 1 500 €/mois 2025 : ARE calculée",
  description: "Intérimaire à 1 500 €/mois en moyenne ? ARE calculée avec primes de précarité et CP inclus. Montant exact et durée d'indemnisation.",
  alternates: { canonical: 'https://www.monchomage.fr/chomage-interimaire-1500-euros' },
}
...
// ── Exemple principal : 1 500 €/mois, 12 mois de missions (365 j) ────────────
const EX_SALAIRE_MENSUEL = 1_500
const EX_MOIS = 12
...
const breadcrumbJsonLd = {
  ...
  itemListElement: [
    ...
    { '@type': 'ListItem', position: 3, name: 'Interimaire 1 500 euros/mois', item: 'https://www.monchomage.fr/chomage-interimaire-1500-euros' },
  ],
}
...
const faqJsonLd = {
  ...
  mainEntity: [
    {
      '@type': 'Question',
      name: "Combien touche un intérimaire à 1 500 €/mois au chômage ?",
      acceptedAnswer: {
        '@type': 'Answer',
        text: `Un interimaire avec un salaire moyen de ${euros(EX_SALAIRE_MENSUEL)}/mois touche ${euros(exAjMensuelle)}/mois au chomage apres 12 mois de missions (${EX_JOURS_TRAVAILLES} jours travailles). L'assiette de calcul est de ${euros(exAssietteMensuelle)}/mois car elle integre la prime de precarite (${Math.round(INTERIM_TAUX_PRECARITE * 100)} %) et l'indemnite de conges payes (${Math.round(INTERIM_TAUX_CP * 100)} %). Le SJR est de ${euros(exSjr)}/j.`,
      },
    },
    {
      '@type': 'Question',
      ...
      acceptedAnswer: {
        '@type': 'Answer',
        text: `Oui. Pour un interimaire, l'assiette de calcul du SJR integre le salaire brut, la prime de precarite (${Math.round(INTERIM_TAUX_PRECARITE * 100)} % du salaire brut) et l'indemnite compensatrice de conges payes (${Math.round(INTERIM_TAUX_CP * 100)} %). Pour un salaire de ${euros(EX_SALAIRE_MENSUEL)}/mois, l'assiette mensuelle est donc de ${euros(exAssietteMensuelle)}/mois.`,
      },
    },
    {
      '@type': 'Question',
      name: "Combien de temps un intérimaire à 1 500 €/mois est-il indemnisé ?",
      acceptedAnswer: {
        '@type': 'Answer',
        text: `La duree d'indemnisation correspond aux jours travailles en missions, dans la limite du plafond d'age. Avec ${EX_JOURS_TRAVAILLES} jours travailles (12 mois de missions), un interimaire de moins de 50 ans est indemnise jusqu'a ${Math.round(ARE_PARAMS.dureeMaxMoins50ansJours / ARE_PARAMS.joursParMois)} mois. Le minimum pour ouvrir des droits est de ${INTERIM_JOURS_MIN} jours travailles sur les 24 derniers mois.`,
      },
    },
  ],
}

export default function PageChomoInterimaire1500() {
  return (
    <>
      ...
      <main className="mx-auto max-w-3xl px-4 py-8 space-y-10">
        ...
        <section className="space-y-4">
          ...
          <p className="text-slate-700">
            ...
            l&apos;assiette r&eacute;elle est de <strong>{euros(exAssietteMensuelle)}/mois</strong>
            ({euros(EX_SALAIRE_MENSUEL)} &times; 1,20), ce qui augmente significativement l&apos;ARE
            par rapport &agrave; ce qu&apos;elle serait sans ces primes.
          </p>
        </section>
              ...
              <tbody>
                <tr className="odd:bg-white even:bg-slate-50">
                  ...
                  <td className="border border-slate-200 px-4 py-2">
                    {EX_MOIS} &times; {euros(EX_SALAIRE_MENSUEL)} = {euros(exTotalBrut)}
                  </td>
                </tr>
                <tr className="odd:bg-white even:bg-slate-50">
                  <td className="border border-slate-200 px-4 py-2 font-medium">
                    + Prime de pr&eacute;carit&eacute; ({Math.round(INTERIM_TAUX_PRECARITE * 100)}&nbsp;%)
                  </td>
                  <td className="border border-slate-200 px-4 py-2">
                    {euros(exTotalBrut)} &times; {INTERIM_TAUX_PRECARITE} = {euros(exPrecarite)}
                  </td>
                </tr>
                <tr className="odd:bg-white even:bg-slate-50">
                  <td className="border border-slate-200 px-4 py-2 font-medium">
                    + Indemn. cong&eacute;s pay&eacute;s ({Math.round(INTERIM_TAUX_CP * 100)}&nbsp;%)
                  </td>
                  <td className="border border-slate-200 px-4 py-2">
                    {euros(exTotalBrut)} &times; {INTERIM_TAUX_CP} = {euros(exCP)}
                  </td>
                </tr>
            ...
            <table className="w-full border-collapse text-sm">
              <tbody>
                <tr className="odd:bg-white even:bg-slate-50">
                  ...
                  <td className="border border-slate-200 px-4 py-2">
                    {EX_JOURS_TRAVAILLES}&nbsp;j ({EX_MOIS}&nbsp;mois de missions)
                  </td>
                </tr>
                <tr className="odd:bg-white even:bg-slate-50">
                  <td className="border border-slate-200 px-4 py-2 font-medium">SJR</td>
                  <td className="border border-slate-200 px-4 py-2">
                    {euros(exAssiette)} &divide; {EX_JOURS_TRAVAILLES} = <strong>{euros(exSjr)}/j</strong>
                  </td>
                </tr>
                <tr className="odd:bg-white even:bg-slate-50">
                  <td className="border border-slate-200 px-4 py-2 font-medium">
                  ...
                  <td className="border border-slate-200 px-4 py-2">
                    {euros(exSjr)} &times; {ARE_PARAMS.tauxFormule1} + {euros(ARE_PARAMS.partieFix)} = {euros(exF1)}/j
                  </td>
                </tr>
                <tr className="odd:bg-white even:bg-slate-50">
                  <td className="border border-slate-200 px-4 py-2 font-medium">
                  ...
                  <td className="border border-slate-200 px-4 py-2">
                    {euros(exSjr)} &times; {ARE_PARAMS.tauxFormule2} = {euros(exF2)}/j
                  </td>
                </tr>
                <tr className="odd:bg-white even:bg-slate-50">
        ...
        <section className="space-y-4">
          ...
          <p className="text-slate-700">
            Votre assiette int&egrave;gre la prime de pr&eacute;carit&eacute; et les CP, ce qui porte l&apos;ARE
            ...
            si ces primes n&apos;&eacute;taient pas int&eacute;gr&eacute;es dans l&apos;assiette.
          </p>
        </section>
        ...
        <section className="rounded-2xl border border-slate-200 bg-slate-50 p-6 space-y-3">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Link href="/chomage-interimaire-2000-euros" className="rounded-lg border border-slate-300 bg-white px-5 py-2 text-sm font-semibold text-slate-700 hover:border-blue-300 hover:text-blue-700">
              Int&eacute;rimaire 2&nbsp;000&nbsp;€/mois
            </Link>
          </div>
        </section>
      </main>
